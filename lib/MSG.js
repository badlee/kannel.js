...
var write = {
	...
	"sms" : function(data, i){
		...
		data = data.concat(this.OCTSTR(typeof i.msgdata == 'undefined' ? null : i.msgdata));
		data = data.concat(this.INTEGER(typeof i.time == 'undefined' ? Math.floor((new Date).getTime()/1000) : i.time));
	...
	"ack" : function(data, i){
		...
		data = data.concat(this.INTEGER(typeof i.time == 'undefined' ? Math.floor(Date.now()/1000) : i.time));
		data = data.concat(this.UUID(typeof i.id == 'undefined' ? true : i.id));
		return data;
	},
...
MSG.prototype = {
	...
	OCTSTR : function(str){
		if(str !== undefined && (str instanceof Array || str instanceof String ) )
			str = new Buffer(str);
	...
	UUID : function(str){
		...
		if(str !== undefined && str instanceof Buffer){
			...
			if(len == -1)
				return null;
			ret = this.readBytes(len).toString();
			return 	ret;
		}
	},
	VOID : function(str){
	...
	"type admin" : function(){
		...
		this.action = this._admin_cmd[this.data['command']] || this._admin_cmd[5];
	...
	"type sms" : function(){
        ...
        this.data['mclass']=this.INTEGER(); /* 0 : normal, 1 : Flash*/
		...
		this.action = this._sms_type[this.data['sms_type']] || this._sms_type[5];
	...
	"type ack" : function(){
        ...
        this.data['time']=this.INTEGER();
        this.data['id']=this.UUID();
		this.action = this._ack_[this.data['nack']] || this._ack_[4];
	},
	...
	_sms_type :["mo","mt_reply","mt_push","report_mo","report_mt",undefined],
	_ack_ : ["success","failed","failed_tmp","buffered",undefined],
	_admin_cmd : ['shutdown','suspend','resume','identify','restart',undefined],
