...
function smsbox(conf,cb){
	...
	if(typeof conf =="string"){
		try{
			...
			var tmp2={},tmp = parser.parseSync(conf.pathname,true);
			this.conf = {};
			for(var i in conf.query)
				tmp2[i] = tmp.path(conf.query[i])[0] || def[i] || "";
			conf = tmp2;
		}catch(e){
...
smsbox.prototype.sendSMS = function(conf){
	...
	if(conf.id){
		...
		this.write("ack",{
			...
			time : conf.time || Math.floor((new Date).getTime()/1000),
