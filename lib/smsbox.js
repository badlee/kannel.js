	...
	if(typeof conf =="string"){
		try{
			...
			for(var i in conf.query)
				try{
					tmp2[i] = tmp.value(conf.query[i]) || def[i] || undefined;
				}catch(e){
					tmp2[i] == undefined
				}
			conf = tmp2;
...
smsbox.prototype.sendUCS2SMS = function(conf){
	conf.coding = 2;
	conf.charset = "UTF-16BE";
	conf.msgdata = new Buffer(conf.msg || conf.text || conf.msgdata);
				...
				if(self.buff.data === null){
					...
					if(self.buff.length > 0 ){
						if(b.length>=(pos+self.buff.length)){
							...
							// J'ajoute les ack si on a recu un sms, pour faciliter les echanges entre l'application et la bearbox
							if(a.type == "sms"){
								var ackSent = false;
								a.data.success = function(){
									if(ackSent) return;// prevent many call
									ackSent  = true;
									self.write('ack',{
